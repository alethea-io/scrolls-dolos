...
impl gasket::framework::Worker<Stage> for Worker {
    ...
    async fn execute(&mut self, unit: &ChainEvent, stage: &mut Stage) -> Result<(), WorkerError> {
        ...
        match unit {
            ChainEvent::Apply(point, record) | ChainEvent::Undo(point, record) => {
                ...
                for module in modules {
                    ...
                    stage.fanout(point.clone(), reduced).await.or_panic()?;
                }
...
#[derive(Stage)]
#[stage(name = "reducers-deno", unit = "ChainEvent", worker = "Worker")]
pub struct Stage {
...
impl Stage {
    async fn fanout(
        &mut self,
        point: Point,
        reduced: Option<serde_json::Value>,
...
impl Config {
    pub fn bootstrapper(self, _ctx: &Context) -> Result<Stage, Error> {
        let reducers: Vec<PathBuf> = self.reducers.iter().map(PathBuf::from).collect();
